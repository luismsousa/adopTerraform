...
export PUBLIC_IP=$(curl -s -H Metadata-Flavor:Google "${METADATA_URL}/network-interfaces/0/access-configs/0/external-ip")
export PRIVATE_IP=$(curl -s -H Metadata-Flavor:Google "${METADATA_URL}/network-interfaces/0/ip")
...
export MAC_ADDRESS=$(curl -s -H Metadata-Flavor:Google "${METADATA_URL}/network-interfaces/0/mac")
